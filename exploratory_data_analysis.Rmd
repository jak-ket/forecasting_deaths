...
forecast_deaths <- function(cases, lag, fat_rate, horizon){
  forecast <- numeric(length=horizon)
  for(h in 1:horizon){
    forecast[h] <- rolling_sum(cases[(length(cases)-lag+h)])/7 * fat_rate
    cases <- c(cases, forecast[h])
  }
  return(forecast)
}
# TODO: 
