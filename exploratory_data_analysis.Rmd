...
fit_params <- function(cases_lagged, deaths_truth, p = c(0.01, 0.025, 1:19/20, 0.975, 0.99)){
  ...
  # ub_size <- 
  # ub_cfr <- 
  opt <- optim(par = c(size = size_start, cfr = cfr_start), lower=lower_bounds, fn = wis_sum)
  
  names(par) <-  # re-name elements  
...
params <- matrix(data = NA, nrow=length(lags), ncol=length(param_names)) 
param_names <- c("size", "cfr")
colnames(params) <- param_names

optvals <- numeric(length = length(lags)) 
