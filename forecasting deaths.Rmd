...
# PROBLEM. ROLSUM ONLY FOR ONE AG

# get filtered and rolsummed data for all age groups
...
calc_forecast_ag <- function(forecast_date, processed_cases, lag, size, cfr, p = c(0.01, 0.025, 1:19/20, 0.975, 0.99)){
...
forecast <- calc_forecast_ag(forecast_date = fc_date, processed_cases = processed_data$cases, lag = ag_best_params["lag"], size = ag_best_params["size"], cfr = ag_best_params["cfr"])
...
# get number of training points for age group with maimal lag
max_lag <- max(best_params[,"lag"])

ag_max_lag <- row.names(best_params)[which.max(best_params[,"lag"])]

prepared_data <- prepare_data(ag = ag_max_lag, cases_frame = cases_wide, deaths_frame = deaths_wide, start = start, end = end, window = window)

max_lag_train_data <- get_lagged_train_data(lag = max_lag, cases_ = prepared_data$cases, deaths_ = prepared_data$deaths, dates_excluded = dates_excluded, start_after_rolsum = start_after_rolsum)
...
for(i in 1:length(ags)){
  ...
  train_data <- get_lagged_train_data(lag = lag, cases_ = processed_data$cases, 
                                      deaths_ = processed_data$deaths,
                                      dates_excluded = dates_excluded, 
                                      start_after_rolsum = start_after_rolsum)
...
# @param cases_frame deaths_frame, start, end, window: params for data preparation
#' @return: quantile levels of pooled forecast distribution
calc_forecast <- function(fc_date, ags, best_params, size_pooled, cases_frame, deaths_frame, start, end, window, levels=c(0.01, 0.025, 1:19/20, 0.975, 0.99)){
  ...
  for(i in 1:length(ags)){
    ...
    # get case rolsum data
    processed_data <- prepare_data(ag = ag, cases_frame = cases_wide, deaths_frame = deaths_wide, start = start, end = end, window = window)
    processed_cases <- processed_data$cases
...
for(i in 1:length(wks_ahead)){
  ...
  fc_quantiles <- calc_forecast(fc_date = fc_date, best_params = best_params, ags = ags, size_pooled = size_pooled, ...)
